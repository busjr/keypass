...
class EditPage(ctk.CTkFrame):
    def __init__(self, parent, main_window):
        ...
        # Применяем цвета 
        self.widget_color = self.theme.get("windows", {}).get("edit", {})

        # параметры из окна
        super().__init__(parent, corner_radius=15, fg_color=self.widget_color["bg"])
        ctk.CTkLabel(self, text="Изменить данные записи", font=("Arial", 18, "bold")).pack(pady=10)
    ...
    def update_info(self, data: dict):
        """ Обновить поля страницы существующими данными """
        self.current_item = data  # сохраним текущие данные для дальнейшего редактирования

        # Очистка и вставка текста
    ...
    def save(self):
        ...
        try:
            ...
            for i, item in enumerate(passwords):
                if (
                    item.get("name") == self.current_item.get("name")
                    and item.get("login") == self.current_item.get("login")
                    and item.get("password") == self.current_item.get("password")
                    and item.get("url") == self.current_item.get("url")
                ):
                    passwords[i] = updated_data  # заменяем старую запись на новую
                    break
            else:
                # если запись не найдена (на всякий случай)
                passwords.append(updated_data)
            ...
            # self.main_window.show_page("home") # TODO добавить начально окно
